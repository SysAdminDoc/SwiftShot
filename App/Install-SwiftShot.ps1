...
.DESCRIPTION
    This installer will:
      ...
      3. Install application dependencies into the venv
...
# -------------------------------------------------------------------
$AppName         = "SwiftShot"
$AppVersion      = Get-AppVersionFromConfig $PSScriptRoot
$PythonMinMajor  = 3
$PythonMinMinor  = 8
$VenvDir         = ".venv"
$LauncherName    = "SwiftShot.cmd"
$IconName        = "swiftshot.ico"
$RequiredModules = @("PyQt5", "PIL", "numpy")
...
if (Test-Path $RequirementsFile) {
    ...
    & $VenvPython -m pip install -r $RequirementsFile --quiet 2>&1
    if ($LASTEXITCODE -ne 0) {
    ...
    foreach ($mod in $RequiredModules) {
        ...
        $pkg = if ($mod -eq "PIL") { "Pillow>=12.2.0" } elseif ($mod -eq "PyQt5") { "PyQt5>=5.15.0" } else { "numpy>=1.24.0" }
        & $VenvPython -m pip install $pkg --quiet 2>&1
        if ($LASTEXITCODE -ne 0) {
...
# Verify imports — write to temp file to avoid PowerShell here-string/python -c newline issues
Write-Step "Verifying imports..."
$verifyTmp = [System.IO.Path]::GetTempFileName() -replace '\.tmp$', '.py'
@'
import sys
try:
    ...
    sys.exit(1)
'@ | Set-Content -Path $verifyTmp -Encoding UTF8

$result = & $VenvPython $verifyTmp 2>&1
...
# Determine python executable path used by generated launchers.
$launcherExe = "%~dp0$VenvDir\Scripts\pythonw.exe"
$vbsPythonExe = "pythonw.exe"
if (-not (Test-Path $VenvPythonW)) {
    $launcherExe = "%~dp0$VenvDir\Scripts\python.exe"
    $vbsPythonExe = "python.exe"
...
WshShell.Run """" & strDir & "\$VenvDir\Scripts\$vbsPythonExe"" """ & strDir & "\main.py""", 0, False
